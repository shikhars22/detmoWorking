...
from clerk import CLERK_API_KEY, CLERK_API_URL, CLERK_ISSUER, CLERK_JWT_KEY, get_clerk_user
...
async def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        payload = jwt.decode(credentials.credentials, CLERK_JWT_KEY, algorithms=["RS256"], issuer=CLERK_ISSUER)
...
async def get_current_user(token_data: dict = Depends(verify_token), db: Session = Depends(get_db)) -> UserDetails:
