...
export function getMatchingCompaniesByEmailDomain(
  user: Pick<UserType, "Email">,
  companies: CompanyDetailsType[],
): CompanyDetailsType[] {
  try {
    const userDomain = user?.Email?.split("@")[1]?.toLowerCase();
    if (!userDomain || !companies) return [];

    return companies.filter((company) => {
      const companyDomain = company?.Email?.split("@")[1]?.toLowerCase();
      return companyDomain && companyDomain === userDomain;
    });
  } catch (error) {
    console.error("Email domain matching error:", error);
    return [];
  }
}
