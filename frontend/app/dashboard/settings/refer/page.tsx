...
const Refer: FC<ReferProps> = ({}) => {
  ...
  const link =
    typeof window !== "undefined"
      ? `${window?.location.origin}/sign-up?ref=${userId}`
      : "";
        ...
        <div className="flex justify-center pt-14 md:pt-0 md:items-center h-[550px]">
          <div className="">
            ...
            <div className="relative h-[45px] w-full">
              <Input
                className="bg-[#F6F6F6] w-full sm:w-[400px] h-full pr-10"
                value={link}
              />
              <Copy
                size={16}
                strokeWidth={1}
                className="text-primary absolute right-2 top-1/2 -translate-y-1/2 cursor-pointer"
                onClick={onCopy}
              />
            </div>
          </div>
